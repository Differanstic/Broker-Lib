...
DB_PATH = os.getenv("DB_PATH", "E:/DB/")
        ...
        if config_path:
            try:
                with open(config_path, 'r') as f:
                    ...
                    self.client_id = fyers_config.get('client_id')
                    self.secret_key = fyers_config.get('secret_key')
                    self.pin = fyers_config.get('pin')
            except Exception as e:
                print(f"Error loading config file: {e}")

        # Fallback to env vars if not found in config
        if not self.client_id: self.client_id = os.getenv('FYERS_CLIENT_ID')
        if not self.secret_key: self.secret_key = os.getenv('FYERS_SECRET_KEY')
        if not self.pin: self.pin = os.getenv('FYERS_PIN')
        
        if not all([self.client_id, self.secret_key, self.pin]):
            raise ValueError("FYERS credentials not found. Please provide a config_path with 'fyers' section or set environment variables.")
...
def load_index(date,exchange,index):
    nifty = pd.read_csv(f'{DB_PATH}{date}/{exchange}/INDEX/{index}.csv',encoding_errors='ignore',on_bad_lines='skip',engine='python')
    try:
        nifty['timestamp'] = (
            nifty['timestamp']
...
def load_option(date,exchange,symbol,option,mob:bool):
    df = pd.read_csv(f'{DB_PATH}{date}/{exchange}/OPTIONS/{symbol}/{option}.csv',encoding_errors='ignore',on_bad_lines='skip',engine='python')
    df['strike'] = option[:-2]
    ...
    try:
        ...
        if mob:
            mob = load_mob(f'{DB_PATH}{date}/{exchange}/OPTIONS/{symbol}-MOB/{option}.csv')
...
def load_option_chain(date,exchange,symbol,mob:bool):
    option_chain = {}
    files = glob.glob(f'{DB_PATH}{date}/{exchange}/OPTIONS/{symbol}/*')
    for f in tqdm(files):
...
def load_stock(date,exchange,symbol,mob:bool):
    stock = pd.read_csv(f'{DB_PATH}{date}/{exchange}/EQUITY/{symbol}.csv',encoding_errors='ignore',on_bad_lines='skip',engine='python')
    try:
        stock['timestamp'] = pd.to_datetime(stock['timestamp'],format='%d/%m/%Y %H:%M:%S:%f',errors='coerce')
        if mob:
            stock_mob = pd.read_csv(f'{DB_PATH}{date}/{exchange}/EQUITY-MOB/{symbol}.csv',encoding_errors='ignore',on_bad_lines='skip',engine='python')
...
def get_dates():
    import glob
    date_list = glob.glob(f'{DB_PATH}*')
    for i in range(len(date_list)) :
        date_list[i] = date_list[i].split('\\')[1]
    if 'Parquet' in date_list:
        date_list.remove('Parquet')
    return date_list